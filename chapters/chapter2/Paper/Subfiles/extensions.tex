...
\caption{\textbf{Trust and Average Returns to Net Wealth with Portfolio-Share Controls}\label{tab:41_avg_shares}}
\endgroup
\begin{threeparttable}
\begingroup
...
\par Figure~\ref{fig:47_actual_returns_by_race_shares} repeats the same exercise with realized returns on the share-augmented estimation samples. As in the main results, pooled OLS, FE, and CRE first average person-year returns within respondent before taking race-specific means, so the comparison remains parallel to the fitted-return figures.

\begin{figure}[!t]
\centering
\begin{subfigure}{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_shares_avg.png}
\caption{Average returns}
\end{subfigure}%
\hfill
\begin{subfigure}{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_shares_pooled.png}
\caption{Pooled OLS}
\end{subfigure}
...
\begin{subfigure}{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_shares_fe.png}
\caption{FE}
\end{subfigure}%
\hfill
\begin{subfigure}{0.45\textwidth}
\centering
\includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_shares_cre.png}
\caption{CRE}
\end{subfigure}
\caption{Race-specific mean actual return minus the overall mean actual return for each specification with portfolio-share controls (subfigure titles). The construction matches Figure~\ref{fig:41_predicted_returns_by_race_shares}, but uses realized returns on the corresponding estimation samples.}
\label{fig:47_actual_returns_by_race_shares}
\end{figure}
% \FloatBarrier

\par The realized-return comparison is useful because it shows whether the fitted racial pattern remains visible after conditioning on portfolio composition and then looking directly at realized outcomes on the same samples. The average-return and pooled panels remain the most directly interpretable for this purpose, while the FE and CRE panels summarize realized returns on the estimation samples used by the panel models.
