...
\par The key takeaway is that \textit{controlling for risk exposure in the CRE model increases explained variability, while retaining the hump-shaped relationship between trust and returns. The estimated persistent component is estimated more accurately in terms of a smaller standard deviation across respondents and a lower share of the total variance attributed to the persistent component.}
...
\par The realized-return comparison in Figure~\ref{fig:47_actual_returns_by_race} is model-free, so I do not repeat it here. The useful comparison in this extension is therefore whether adding portfolio shares materially changes the fitted race pattern relative to that common realized-return benchmark.
