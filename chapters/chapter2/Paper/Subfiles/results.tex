...
\par Figure~\ref{fig:47_avg_returns_hist} shows the distribution of respondent-level average returns to net wealth in the final average-return estimation sample. Even after the 5\% winsorization used throughout the main analysis, the cross section remains dispersed, which helps motivate the move from average returns to the person-year pooled specification.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\textwidth]{../../Code/Analysis/Figures/47_avg_returns_hist.png}
  \caption{Distribution of average returns to net wealth in the final average-return sample.}
  \label{fig:47_avg_returns_hist}
\end{figure}
\FloatBarrier
...
\par The next step is to allow for individual fixed effects in the panel regression so that the error term $\varepsilon_{it}$ can be written as \[\varepsilon_{it} = \alpha_{it} + e_{it},\] where $e_{it}$ may possibly be serially correlated. Notably, \textit{the predictive power of any the time-invariant regressors (trust, education, race, gender, region, immigration status) will be absorbed by the individual fixed effect term}. 
...
\par By choosing to model the hump-shaped relationship between trust and returns using a combination of linear and quadratic terms, I can describe the level of trust which maximizes returns to net wealth in the following way. Starting from
\[
r^{net}_{it} = \alpha_i + \lambda_t + X_{it}'\beta + \varepsilon_{it},
\]
...
\subsection{Predicted vs Actual returns by race}

\par An important finding in the HRS data is that i) the \texttt{NH Black} racial category predicts trust well, ii) regressions of returns to net wealth on race without trust show nontrivial racial differences, and iii) once trust (and trust$^2$) enter the return regressions, those differences attenuate. Table~\ref{tab:47_race_no_trust} summarizes the no-trust race coefficients for the average-return and pooled specifications using the reduced and full control sets.

\InputStataRegTableCompact{\textbf{Race Coefficients in No-Trust Average-Return and Pooled Specifications}}{tab:47_race_no_trust}{../../Code/Analysis/Tables/47_table1_race_no_trust.tex}{Columns~\textit{(1)--(2)} are cross-sectional OLS of 5\% winsorized average returns to net wealth; columns~\textit{(3)--(4)} are pooled person-year OLS of 5\% winsorized returns to net wealth with standard errors clustered by household. In each model family, the reduced column includes race together with the baseline return controls, while the full column adds the extended demographic controls. Only race coefficients are displayed.}
\FloatBarrier

\subsubsection{Predicted returns by race}
...
\par In each of the specifications, the mean predicted returns are higher on average for \texttt{NH Black} respondents. In the pooled OLS and CRE versions of the model, the trust terms are estimated directly, and these are the same figures that show the clearest positive deviation in mean predicted returns for Black respondents. The FE model, which does not estimate the effect of time-invariant trust on returns, produces a deviation closer to zero (though still positive). No other racial categories show a robust, positive deviation from mean predicted returns.
...
\begin{figure}[H]
  ...
  \caption{Race-group deviations from the overall mean predicted return by specification.}
