...
\par Figure~\ref{fig:47_actual_returns_by_race} repeats the same comparison using realized returns rather than fitted values. For the panel specifications, I restrict to each model's estimation sample and then average person-year returns within respondent before taking race-specific means, so the construction is directly parallel to the predicted-return figures.

\begin{figure}[H]
  \centering
  \begin{subfigure}{0.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_avg.png}
    \caption{Average returns}
  \end{subfigure}\hfill
  \begin{subfigure}{0.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_pooled.png}
    \caption{Pooled OLS}
  \end{subfigure}

  \begin{subfigure}{0.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_fe.png}
    \caption{FE}
  \end{subfigure}\hfill
  \begin{subfigure}{0.45\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../../Code/Analysis/Figures/47_actual_returns_by_race_cre.png}
    \caption{CRE}
  \end{subfigure}

  \caption{Race-specific mean actual return minus the overall mean actual return for each specification (subfigure titles). The construction mirrors Figure~\ref{fig:38_predicted_returns_by_race}, but replaces fitted values with realized returns on the same estimation samples.}
  \label{fig:47_actual_returns_by_race}
\end{figure}

\par The realized-return comparison is useful as a check on the fitted-return evidence. The same broad ranking can be seen in the average-return and pooled panels, while the FE and CRE panels summarize realized returns on the samples used to estimate those models rather than an explicit decomposition of the race effect itself.
